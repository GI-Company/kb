...
- **Sandbox sophistication.** `api/exec.ts`'s command allowlist had to shrink further than the original Go version's, since Vercel's Node function runtime doesn't ship the broader toolset a real host would (no git/python/go/rust/ffmpeg by default). Guessing which commands survive turned out to be unreliable: an early list of 39 contained 12 that could never run, and the gap was only found by probing the deployed function one command at a time. Commands genuinely needed but absent — `curl`, `dig`, `ping` — were reimplemented natively rather than shelled out to.
