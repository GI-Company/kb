...
// Local-model tool-call contract (see lib/agents.ts's BNLM_TOOL_CONTRACT):
// an agent that wants to train/run BNLM emits a fenced ```tool block
// containing one JSON object like {"tool":"bnlm.train","args":{...}}.
...
/** Executes a bnlm.* tool call against the in-browser LocalModelService and reports the outcome back via `publishResult`. */
async function runLocalModelTool(toolCall: ToolCall, publishResult: (content: string) => void) {
    try {
        if (toolCall.tool === 'bnlm.train') {
            ...
            publishResult(
                `🧠 Trained a local model right here in your browser: ${init.paramCount.toLocaleString()} params, ` +
                `vocab ${init.vocabSize}, ${train.steps} steps, final loss ${train.finalLoss.toFixed(3)}. ` +
                `Open the **Local Model** app to keep training it or generate from it directly.`
            );
        } else if (toolCall.tool === 'bnlm.generate') {
            const { prompt = '', maxTokens = 60 } = toolCall.args || {};
            if (!localModel.isReady) throw new Error('No local model has been trained yet in this tab — ask me to train one first.');
            ...
            publishResult(`🧠 Local model output:\n\n${text}`);
        } else {
            publishResult(`⚠️ Unknown local model tool: "${toolCall.tool}"`);
        }
    } catch (err: any) {
        publishResult(`⚠️ Local model tool failed: ${err?.message || err}`);
    }
}
...
export const AIChatApp: React.FC = () => {
    ...
    // Reads agentsRef (kept in sync below) rather than `agents` directly —
    // these closures are created once on mount and would otherwise always
    // see the initial empty array, permanently forcing direct mode.
    useEffect(() => {
        ...
        const unsub = kernel.subscribe((env: Envelope) => {
            ...
            if (env.topic === 'agent.chat:reply') {
                ...
                setStreamBuffer(prev => {
                    ...
                    if (toolCall) {
                        runLocalModelTool(toolCall, (content) => {
                            setMessages(msgs => [...msgs, {
                                id: Math.random().toString(36),
                                role: 'agent',
                                content,
    ...
    const sendMessage = () => {
        ...
        if (directMode) {
            ...
            // `messages` here is still the pre-update state (setState above
            // hasn't re-rendered yet) — exactly the prior turns, not
            // including the user message just sent as `msg`.
            const history = messages.slice(-10).map(m => ({ role: m.role, content: m.content }));
            if (imageBase64) {
                kernel.sendToAgent(selectedAgent, 'agent.chat', { msg, image: imageBase64, history });
            } else {
                kernel.sendToAgent(selectedAgent, 'agent.chat', { msg, history });
            }
        }
